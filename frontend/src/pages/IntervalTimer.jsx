...
// routines accessible from this page (as opposed to the main lifting sessions).
...
export default function IntervalTimer() {
  ...
  const routineIdRef = useRef(null)  // for completion recording inside interval callback
  ...
  useEffect(() => {
    function onVisibilityChange() {
      ...
      if (ctxRef.current?.state === 'suspended') {
        ctxRef.current.resume().then(() => console.log('[IntervalTimer] AudioContext resumed'))
      }
  ...
  function startDisplayInterval(offset = 0) {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (elapsed >= endTimeRef.current) {
        ...
        // Record completion date (only when timer finishes naturally)
        if (routineIdRef.current) {
          const today = new Date().toISOString().split('T')[0]
          recordCoreRoutineCompletion(routineIdRef.current, today)
        }
        return
      }
  ...
  function handleStart(offset = 0) {
    ...
    if (offset === 0) {
      ...
      routineIdRef.current = selectedRoutine.id
    }
    ...
    console.log(`[IntervalTimer] scheduled ${nodesRef.current.length} nodes, offset=${offset.toFixed(1)}s`)
  ...
  if (timerState === 'idle') {
    return (
      <div className="core-timer idle">
        ...
        <button className="core-btn core-btn-select" onClick={() => navigate('/interval/select')}>
          Select Workout
        </button>
      </div>
    )
  ...
  const isRest = display.type === 'rest'
  const showCounter = isWork || isRest
  ...
  return (
    <div className="core-timer running">
      ...
      {/* Exercise counter — visible during work and rest for layout stability */}
      <p className={`core-timer-counter${showCounter ? '' : ' invisible'}`}>
        {showCounter
          ? `${display.exerciseIndex} / ${exCountRef.current}${isRest ? ' (Rest)' : ''}`
          : '\u00A0'}
      </p>
